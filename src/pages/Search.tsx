...
const Search = () => {
  ...
  useEffect(() => {
    // Fetch Movie Genres for the sidebar
    tmdbApi.get('/genre/movie/list').then(res => setGenres(res.data.genres));
  }, []);
    ...
    <div className="flex flex-col md:flex-row gap-6">
      ...
      <div className="flex-1">
        <form onSubmit={handleSearch} className="mb-8 flex gap-2">
          ...
          />
          <button type="submit" className="bg-indigo-600 px-6 py-3 rounded font-bold hover:bg-indigo-700">Search</button>
        </form>

        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
          {filteredResults.map(media => (
             <Link to={`/details/${media.media_type || 'movie'}/${media.id}`} key={media.id} className="bg-gray-800 rounded-lg overflow-hidden shadow-lg hover:scale-105 transition-transform">
              <img 
                src={media.poster_path ? (media.poster_path.startsWith('http') ? media.poster_path : `https://image.tmdb.org/t/p/w500${media.poster_path}`) : '/placeholder.jpg'} 
