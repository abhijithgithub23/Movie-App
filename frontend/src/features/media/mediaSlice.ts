...
export const getTrending = createAsyncThunk<Media[]>(
  "media/getTrending",
  async () => {
    // Points to http://localhost:5000/api/media/trending
    const res = await tmdbApi.get("/media/trending");
    return res.data.results; 
  }
);
...
export const getMovies = createAsyncThunk<{ results: Media[]; page: number }, number | void>(
  ...
  async (pageArg) => {
    const page = typeof pageArg === "number" ? pageArg : 1;
    // Points to http://localhost:5000/api/media/movies
    const res = await tmdbApi.get("/media/movies", { params: { page } });
    return { results: res.data.results, page };
  }
);
...
export const getTVShows = createAsyncThunk<{ results: Media[]; page: number }, number | void>(
  ...
  async (pageArg) => {
    const page = typeof pageArg === "number" ? pageArg : 1;
    // Points to http://localhost:5000/api/media/tv
    const res = await tmdbApi.get("/media/tv", { params: { page } });
    return { results: res.data.results, page };     
  }
);
