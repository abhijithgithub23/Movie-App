...
const MediaCard = memo(({ m }: { m: MediaItem }) => {
  // FIX: Smart Image URL logic!
  const getImageUrl = (path?: string) => {
    if (!path) return null;
    if (path.startsWith('http') || path.startsWith('data:')) return path; // Use Cloudinary/Custom URL directly
    return `https://image.tmdb.org/t/p/w500${path}`; // Append TMDB base URL for API movies
  };

  const imgSrc = getImageUrl(m.poster_path);
  ...
  return (
    <Link to={`/details/${m.media_type || 'movie'}/${m.id}`} className="group relative bg-card-bg rounded-2xl overflow-hidden border border-text-muted/20 hover:border-btn-bg/50 transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-btn-bg/20">
      ...
      <div className="aspect-[2/3] w-full bg-main/50 relative overflow-hidden">
        {imgSrc ? (
          <img src={imgSrc} alt={m.title || m.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
        ) : (
